...
function runTheCalc(myEntry) {
    if (myEntry == "C") {
        ...
        operand = myEntry;
        if (runningTotalFlag == 1) {
            // first calculate
            console.log("Running total is being calculated!");
            sum = parseFloat($('#summation').text());
            console.log("running addend: " + MyAddend);
            console.log("running sum for operation: " + sum);
            sum = runningSum;
            $('#summation').text("0");
            $('#addend').text("Running total:" + runningSum);
            //we should be set up for next entry, a number to continue the running total.
        } else {
            MyAddend = sum;
            $('#addend').text(myEntry + " " + MyAddend);
            runningSum = Calculate(operand, MyAddend, sum);
            sum = 0;
            runningTotalFlag = 1;
            $('#summation').text("0");
        }
